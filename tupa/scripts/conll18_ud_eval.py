...
def load_conllu(file):
    ...
    class UDRepresentation:
        def __init__(self):
            ...
            # List of UDSpan instances with start&end indices into `characters`.
            self.sentences = []
            # List of empty UDWord instances.
            self.empty = []
    ...
    class UDWord:
        def __init__(self, span, columns, is_multiword):
            ...
            self.parent = self.enhanced = None
        ...
        if not line:
            ...
            def process_word(word):
                if word.parent == "remapping":
                    raise UDError("There is a cycle in a sentence: " + " ".join(word.columns))
...
def evaluate(gold_ud, system_ud):
    ...
    def alignment_score(alignment, key_fn=None, filter_fn=None, count_fn=None):
        if count_fn is not None:
            gold = sum(count_fn(gold) for gold in alignment.gold_words if filter_fn is None or filter_fn(gold))
            system = sum(count_fn(system) for system in alignment.system_words if filter_fn is None or filter_fn(system))
            aligned = sum(count_fn(word.gold_word) for word in alignment.matched_words if filter_fn is None or filter_fn(word.gold_word))
        ...
        for words in alignment.matched_words:
            if filter_fn is None or filter_fn(words.gold_word):
                key_gold = key_fn(words.gold_word, gold_aligned_gold)
                key_system = key_fn(words.system_word, gold_aligned_system)
                if key_gold == key_system:
                    correct += 1 if count_fn is None else len(set(key_gold).intersection(key_system))
