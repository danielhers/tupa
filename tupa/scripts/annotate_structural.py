...
def main(args):
    ...
    features_passage_by_id = {hack_id(passage.ID): passage for passage in get_passages_with_progress_bar(
        args.features_source_dir, desc="Reading passages for structure features")}
    for passage in get_passages_with_progress_bar(args.source_dir, desc="Reading source passages"):
        try:
