...
class Edge:
    ...
    """
    def __init__(self, parent, child, tag, refinement=None, remote=False):
        ...
        self.refinement = refinement # String refinement tag
    ...
    def __repr__(self):
        return Edge.__name__ + "(" + self.tag + "-" + self.refinement + ", " + repr(self.parent) + ", " + repr(self.child) +\
               ((", " + str(self.remote)) if self.remote else "") + ")"

    def __str__(self):
        return "%s -%s-%s-> %s%s" % (self.parent, self.tag, self.refinement, self.child,
    ...
    def __hash__(self):
        return hash((self.parent.index, self.child.index, self.tag, self.refinement))
